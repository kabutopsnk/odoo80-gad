...
class l10n_ec_hr_he_head_import(osv.osv_memory):

    _name='wizard.import.hr.he.head'
    _description='Asistente para importar Marcaciones desde XLS'
    ...
    def import_sheet_dat(self, cr, uid, ids, context={}):
        emp_obj = self.pool.get('hr.employee')
        data = self.read(cr, uid, ids)[0]
        line_obj = self.pool.get('hr.mark')
        #obj = self.pool.get('hr.he.head')
        ...
        if data['archivo']:
            arch = data['archivo']
            arch_dat = base64.b64decode(arch)
            lines = arch_dat.split("\n")
            #lines = lines[2:]
            for line in lines:
                #data = arch_dat.readline().split("\t")
                data = line.split('\t')
                emp_id = emp_obj.search(cr, uid, [('name','=',data[0])])
                if emp_id:
                    line_obj.create(cr, uid, {
                                              'employee_id':emp_id[0],
                                              'datetime_mark':data[1],
                                              'reloj':data[2],
                    ...
                    if emp_id:
                        for emp in emp_id:
                            ...
                            line_obj.create(cr, uid, {
                                                      ...
                                                      'datetime_mark':sh.cell(r,2).value,
                                                      'reloj':sh.cell(r,0).value,
                                                      #'rule_id':parent.rule_id.id,
                                                      #'head_id':parent.id,
                                                      #'date':parent.date,
                                                      #'biweekly':parent.biweekly,
